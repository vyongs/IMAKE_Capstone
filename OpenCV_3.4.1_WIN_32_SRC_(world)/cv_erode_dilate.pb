...
#CV_DESCRIPTION = "Erodes / Dilates an image using a specific structuring element." + #LF$ + #LF$ +
                  "TRACKBAR    " + #TAB$ + ": Adjust the iterations." + #LF$ +
                  "SPACEBAR    " + #TAB$ + ": Change the element."
    ...
    *image.IplImage = cvLoadImage(ImageFile, #CV_LOAD_IMAGE_ANYDEPTH | #CV_LOAD_IMAGE_ANYCOLOR)
    ...
    If *resize\width > 200 And *resize\height > 200
      ...
      *param\Value = window_handle
      ...
      Repeat
        If *resize
          If (nPosition - #MAX_ITERS) < 0
            *kernel = cvCreateStructuringElementEx((nPosition - #MAX_ITERS) * -1 * 2 + 1, (nPosition - #MAX_ITERS) * -1 * 2 + 1, (nPosition - #MAX_ITERS) * -1, (nPosition - #MAX_ITERS) * -1, shape, #Null)
            cvErode(*reset, *resize, *kernel, 1)
            cvDilate(*reset, *resize, *kernel, 1)
          Else
            *kernel = cvCreateStructuringElementEx((nPosition - #MAX_ITERS) * 2 + 1, (nPosition - #MAX_ITERS) * 2 + 1, (nPosition - #MAX_ITERS), (nPosition - #MAX_ITERS), shape, #Null)
            cvDilate(*reset, *resize, *kernel, 1)
            cvErode(*reset, *resize, *kernel, 1)
          EndIf
