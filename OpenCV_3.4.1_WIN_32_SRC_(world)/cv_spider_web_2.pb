...
#CV_DESCRIPTION = "Draw a Spider Web sketch from a grayscale image." + #LF$ + #LF$ +
...
Procedure TestColor(*resize.IplImage, nGray, nLocation)
  nPercent.f = nGray / PeekA(@*resize\imageData\b + nLocation) * 100
        ...
        If Not nStop
          ...
          For rtnCount = 0 To nSpeed - 1
            If NewPixel Or nTries > #MAX_TRIES
              ...
              nLocation = Random(*resize\imageSize)
              x1 = nLocation % *resize\widthStep
              y1 = nLocation / *resize\widthStep
              nGray = PeekA(@*resize\imageData\b + nLocation)
            EndIf
            nLocation = Random(*resize\imageSize)

            If TestColor(*resize, nGray, nLocation)
              x2 = nLocation % *resize\widthStep
              y2 = nLocation / *resize\widthStep

              If Sqr((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)) <= nDistance
                cvLine(*spiderweb, x1, y1, x2, y2, nGray, 0, 0, 0, 1, 8, #Null) : nCount + 1
              EndIf
            EndIf
