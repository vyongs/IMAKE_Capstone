...
#CV_DESCRIPTION = "Apply a black and white posterize effect to an image." + #LF$ + #LF$ +
...
Procedure PosterizeImage(*src.IplImage, *dst.IplImage, nThreshold, Array arrTile(2))
  Define.PixelColor *black, *white
  *black = AllocateMemory(SizeOf(PixelColor))
  *white = AllocateMemory(SizeOf(PixelColor))
  ...
  *white\b = 255 : *white\g = 255 : *white\r = 255
  ...
  For y = 0 To *src\height - 1
    For x = 0 To *src\width - 1
      ...
      Else
        PixelDraw(*dst, x, y, *white)
        nValue = Color2Gray(PixelSurvey(*src, x, y)) / 24

				If arrTile(nValue, (y % 4) * 4 + x % 4) : PixelDraw(*dst, x, y, *white) : Else : PixelDraw(*dst, x, y, *black) : EndIf
      ...
      Repeat
        If *posterize
          If nThreshold
            ...
            PosterizeImage(*smooth, *posterize, nValue, arrTile())
