...
class Virus:
    ...
    def update(self):
        ...
        if (exam_mask[v.x][v.y] == 255) and (self.touched_B==False):
        ##사람 없고 && 터치된 기록 x
            self.touched_B = False
            
        elif (exam_mask[v.x][v.y] == 255) and (self.touched_B== True):
        ##사람 없고 && 터치된 기록 o
            self.touched_B = False
            
        elif (exam_mask[v.x][v.y] == 0) and (self.touched_B==False):
        ##사람 있고 && 터치된 기록 x
            self.touched_B = True
            self.touched_N += 1
            self.radius-=10

        elif (exam_mask[v.x][v.y] == 0) and (self.touched_B==True):
        ##사람 있고 && 터치된 기록 o
            self.touched=True
...
##            print("done")
  ...
  ##--------------------게임의 상태를 업데이트하는 부분--------------------------
        ...
        masking = np.zeros((W,H,3))
        masking[temp != 0] = [0,0,255] #사람o
        masking[temp == 0] = [0,0,0] #사람x

        exam_mask = np.flipud(temp)
