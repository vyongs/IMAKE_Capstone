...
rain_drops['growth'] = np.random.uniform(1, 3, n_drops)
print(rain_drops)
...
#        abdiff = cv2.morphologyEx(abdiff, cv2.MORPH_CLOSE, kernel, 2)
        ...
        tempMask = np.array(thresh_img)
        exists_x = np.where(tempMask==255)[0]/240
        exists_y = np.where(tempMask==255)[1]/320
        myLen = len(exists_x)
        if myLen==0:
            rain_drops['position'][current_index] = np.random.uniform(0, 1, 2)
        else:
            myPoint = random.randint(0, myLen-1)
            rain_drops['position'][current_index] =[exists_y[myPoint],exists_x[myPoint]]            
            
        rain_drops['size'][current_index] = 5
        rain_drops['color'][current_index] = (0, 0, 0, 255)
        rain_drops['growth'][current_index] = np.random.uniform(1, 3)
        ###################DRAWING##############################################
